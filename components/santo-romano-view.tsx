...
export interface SantoRomanoData {
  date: string;
  dateLabel: string;
  title: string;
  grado: string;
...
export interface SantoRomanoViewProps {
  date: string;
  onDateChange?: (date: string) => void;
  isEmbedded?: boolean;
}

export function SantoRomanoView({
  date,
  onDateChange,
  isEmbedded = false,
}: SantoRomanoViewProps) {
  const [data, setData] = useState<SantoRomanoData | null>(null);
  ...
  // Fetch dati del Santo Romano da ChiesaCattolica.it (CEI)
  const fetchSanto = useCallback(async (dateStr: string) => {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch(`/api/santo-del-giorno?date=${dateStr}&rite=romano`);
      if (!res.ok) {
        throw new Error(`Impossibile caricare il Santo del Giorno (${res.status})`);
      }
      const json: SantoRomanoData = await res.json();
      setData(json);
    } catch (err: unknown) {
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <h1 className="font-serif text-2xl sm:text-3xl font-normal text-[#3f3933]">
                Santo del Giorno (Rito Romano)
              </h1>
              <p className="text-xs sm:text-sm text-[#7d6b58] mt-1 font-serif italic">
                Commemorazioni ufficiali della Chiesa Cattolica Italiana (Conferenza Episcopale Italiana - CEI)
              </p>
            </div>
      ...
      {loading ? (
        <div className="rounded-3xl border border-[#e2d5c4] bg-[#fdfbf7] p-16 text-center shadow-sm space-y-4">
          ...
          <p className="font-serif text-lg text-[#aa9576]">
            Caricamento del Santo del Giorno da ChiesaCattolica.it (CEI)...
          </p>
        </div>
      ) : error ? (
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
              ...
              <div className={`${data.imgUrl ? "md:col-span-7" : "md:col-span-8"} space-y-4`}>
                ...
                <div className="flex flex-wrap items-center gap-2.5 pt-2">
                  ...
                  <Link
                    href={`/liturgia?rite=romano&date=${date}`}
          ...
          {/* Sezione Altri Santi e Beati del Giorno con sotto-immagini */}
