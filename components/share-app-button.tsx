...
export function ShareAppButton({ variant = "header", className = "" }: ShareAppButtonProps) {
  ...
  if (variant === "header") {
    return (
      <button
        onClick={handleShare}
        className={`inline-flex items-center justify-center gap-1.5 rounded-full border border-[#d7c7b5] bg-[#ede4d8] px-2.5 sm:px-3 py-1.5 sm:py-1 text-xs font-semibold text-[#5c4a37] hover:bg-[#e4d7c7] active:scale-95 transition shadow-xs cursor-pointer ${className}`}
        title="Condividi Note di Fede con cantori e parrocchiani"
        aria-label="Condividi Note di Fede"
      >
        {copied ? (
          <>
            <span className="text-emerald-700 font-bold text-xs">✓</span>
            <span className="text-emerald-700 font-bold hidden sm:inline">Link Copiato!</span>
          </>
        ) : (
          <>
            <svg className="h-4 w-4 sm:h-3.5 sm:w-3.5 text-[#5c4a37] shrink-0" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" />
            </svg>
            <span className="hidden sm:inline">Condividi App</span>
          </>
        )}
      </button>
    );
